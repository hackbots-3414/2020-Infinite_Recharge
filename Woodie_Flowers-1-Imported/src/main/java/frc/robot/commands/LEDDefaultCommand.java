...
public class LEDDefaultCommand extends CommandBase {
  ...
  public void execute() {
    ...
    if (!DriverStation.isAutonomous() && matchTime <= 10 && intPart % 2 == 1) {
      System.out.println("yellow");
        ledSubsystem.switchOnLEDs(ColorSystem.COLOR_YELLOW, ColorSystem.COLOR_PATTERN_SOLID);
    } else if (!DriverStation.isAutonomous() && matchTime <= 16 && matchTime >= 10 && intPart % 3 == 0) {
      System.out.println("blue");
      ledSubsystem.switchOnLEDs(ColorSystem.COLOR_LIGHT_BLUE, ColorSystem.COLOR_PATTERN_SOLID);
    } else if (!DriverStation.isAutonomous() && matchTime <= 30 && matchTime >= 16 && intPart % 4 == 1) {
  ...
  public double getMatchTimeRemaining() {
    // return 150 - DriverStation.getMatchTime();

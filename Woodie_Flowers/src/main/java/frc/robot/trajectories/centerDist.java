...
public class centerDist extends CommandBase {
  ...
  public RamseteCommand ramseteCommand = new RamseteCommand(TrajectoryGenerator.generateTrajectory(
    new Pose2d(0,0, new Rotation2d(0)), List.of(), new Pose2d(-1.4, 0, new Rotation2d(0)), configReversedTrue), m_drivetrainSubsystem::getPose, disabledRamsete, new SimpleMotorFeedforward(DriveConstants.ksVolts,
...
.setReversed(false);
          ...
          Trajectory backwards = TrajectoryGenerator.generateTrajectory(
            new Pose2d(0,0, new Rotation2d(0)), List.of(), new Pose2d(-1.4, 0, new Rotation2d(0)), configReversedTrue);
            RamseteController disabledRamsete = new RamseteController() {
              @Override
              public ChassisSpeeds calculate(Pose2d currentPose, Pose2d poseRef, double linearVelocityRefMeters,
                      double angularVelocityRefRadiansPerSecond) {
